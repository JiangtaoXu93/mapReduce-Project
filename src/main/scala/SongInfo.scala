/**
  * Created by Patrizio on 2017/11/13.
  */
class SongInfo(row:String) extends java.io.Serializable{
  ...
  def getArtistName(): String = line(17).toLowerCase().replaceAll("and", "").replaceAll("vs", "").replaceAll("ft", "").replaceAll("with", "").replaceAll("featuring", "").replaceAll("feat", "").replaceAll("\\s", "")
...
//  }
  ...
  def checkValidity(): Boolean = {
    ...
    if (isValidRow
      && !(getTrackId().equals(""))
      && !(getTitle().equals("")) && !(getSongId().equals(""))  && !(getArtistName.equals(""))) {
      result = true
    }
    result
  }

  def getMetric(metric: String): Double = {
    metric match {
